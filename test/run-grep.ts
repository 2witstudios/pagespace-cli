...
const api = new PageSpaceApi({ apiUrl, authToken, defaultDriveSlug: drive, mountPrefix: "pagespace", modelPageId: undefined });
...
async function main(): Promise<void> {
  ...
  ok("ignoreCase matches '# Overview'", /overview:\d+: .*Overview/i.test(t3) && t3 !== "No matches found", JSON.stringify(t3.slice(0, 80)));
  ...
  ok("literal search runs without regex error", Array.isArray(r4.content) && typeof r4.content[0].text === "string");
